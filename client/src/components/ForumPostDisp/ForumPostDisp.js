...
const ForumPostDisp = _ => {
  ...
  const { newsForumPost, handleGetComments, comments, handleAddComment, handleInputChange, comment } = useContext(ForumContext)
        ...
        <AccordionDetails >
          ...
          </Typography>
          {
            comments.map(comment =>
              <Card className={classes.root}>{comment.comment}</Card>
              )
          }
        <TextField id="outlined-basic"  name="comment" value={comment} label="comment" variant="outlined" onChange={handleInputChange} />
        <p>
          <Button id={post._id} onClick={handleAddComment} variant="contained" color="primary">
            Comment
          </Button>
        </p>
        </AccordionDetails>
